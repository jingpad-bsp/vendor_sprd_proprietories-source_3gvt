...
class RecorderThread extends HandlerThread
    ...
    void stopWork() {
        ...
        synchronized (m_StopEncLock) {//UNOSOC:modify for bug1169974,1169975,1169976
            while (m_bEncAudio){
                m_bEncAudio = false;
                ...
                switch (msg.what) {
                    case VTManager.MSG_RECORDER_ENABLE:
                        ...
                        synchronized (m_StopEncLock) {
                            if (m_bEncAudio) {
                                log("m_bEncAudio already recording so return");
                                return;
                            }
                        }
                        synchronized (m_StopVideocLock) {
                           if (m_bVideoStarted) {
                                log("m_StopVideocLock already recording so return");
                                return;
                            }
                        }
    ...
    void enableRecord(boolean bEnable, int type, String fn) {
        ...
        synchronized (m_ThreadLock) {
            try {
                ...
                while (mHandler == null && bEnable) {
                    m_ThreadLock.wait();
                }
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            if (bEnable && mHandler != null) {
            ...
            public void run() {
                ...
                for (;;) {
                    ...
                    if (inputBufferIndex >= 0) {
                        synchronized (m_GetBuffer) {
                            vlog("startVoiceEncode(), m_RingBuffer.size(): " + m_RingBuffer.size());
